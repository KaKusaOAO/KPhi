import com.kakaouo.viewer.phi.model.NoteModel
import com.kakaouo.viewer.phi.polyfill.audio.AudioBuffer

class CatchNote : Note(NoteType.CATCH) {
    companion object {
        fun deserialize(model: NoteModel): CatchNote {
            val result = CatchNote()
            result.populateFromModel(model)
            return result
        }
    }

    override val audioFx: AudioBuffer?
        get() = game!!.assets.audio("catchFx")

    override fun render() {
        super.render()
        val game = game!!
        if (isOffscreen() && !game.config.offScreenForceRender) return

        val texture = game.assets.image(if (hasSibling) "catchHL" else "catch")!!
        drawSimpleNote(texture)
    }
}